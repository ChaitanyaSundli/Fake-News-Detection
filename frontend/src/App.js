...
export default function App() {
  ...
  const handleModelChange = async (e) => {
    ...
    try {
      ...
      alert(`✅ ${modelName.replace('.pt','').toUpperCase()} model loaded successfully!`);
    } catch (error) {
  ...
  // Helper function for result card color (gradient logic)
  const getResultBackground = (confidence) => {
    if (confidence > 0.85) return '#dcfce7'; // Green
    if (confidence > 0.6) return '#fef9c3';  // Yellow
    if (confidence > 0.4) return '#fde68a';  // Orange
    return '#fee2e2';                        // Red
  };
        ...
        {page === 'predict' && (
          <>
            ...
            <div className="step">
              ...
              <p className="subtext">🔧 Loaded model: {selectedModel.replace('.pt', '').toUpperCase()}</p>
            </div>
