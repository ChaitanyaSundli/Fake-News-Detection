  ...
  const handleTrain = async () => {
    ...
    try {
      ...
      eventSource.onmessage = (e) => {
        ...
        if (
          line.includes("Epoch") || line.includes("Training Loss:") ||
          line.includes("Validation Loss:") || line.includes("Validation Accuracy:") ||
          line.includes("ROC-AUC") || line.includes("F1") || line.includes("Cost-Benefit Analysis:")
        ) {
      ...
      <motion.div className="card">
        ...
        {step === 3 && (
          <motion.div className="step">
            ...
            <div className="chart-selector">
              ...
              <select id="chart-select" value={selectedChart} onChange={handleChartChange} className="select">
                ...
                {trainingImages.map((img) => {
                  const chartName = img.split('.')[0].replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase());
                  return (
            ...
            <button onClick={() => { setTrainingImages([]); setLogLines([]); setStep(1); }} className="btn-purple">
              🔄 Reset Wizard
            </button>
            ...
            {predictionResult && (
              <>
                <motion.div className="result-card" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                  <h3>{predictionResult.prediction === 'FAKE' ? '🚨 FAKE NEWS DETECTED' : '✅ REAL NEWS'}</h3>
                  ...
                  {predictionResult.processed_text && (
                    <p className="subtext">🔍 {predictionResult.processed_text}</p>
                  )}
                </motion.div>
                ...
                <button className="btn-green" onClick={() => {
                  setUserInput('');
                  setPredictionResult(null);
                  alert('🧹 Prediction and input cleared.');
                }}>
                  🧹 Clear Result
                </button>
